...
## Analysis
psuedo c code for `<login>` function, manually decompiled assembly code from [here](./dump.asm):
```c
void login() {
...
For both `username` and `password`, `malloc` is called with `0x10` as the size. However, `gets` is called with `0x30` as the size, which is larger than the allocated size.
This means that we can overflow the heap buffer for both `username` and `password`.

**Vulnerability**: heap buffer overflow

### Data Structures and Memory Layout for Heap
From the assembly code for `<malloc>`, we can see the memory section for `0x2400` should probably be the heap.
...
psuedo c code for `<free>` function, manually decompiled assembly code from [here](./dump.asm):
```c
...
struct chunk {
  ...
  union {
    uint16_t size;
    unsigned is_allocated:1;  // specifies if the current chunk is allocated or not
  };
  char data[0];
};
```
...
> For both chunks, the `size` field is `0x21` when allocted and suggested that the chunk size is `0x20`. However, both chunks are occupied `0x16` bytes, which is `0x10` bytes for the data and `0x6` bytes for the metadata. It seems like the size value has beed calculated s.t. it is `0x10` aligned but the **actual** size of the chunk is not aligned.
</details>
...
Since **FAKE_CHUNK_1** is free, if it is the `fd` or `bk` chunk from the <u>to be freed chunk</u>, it will be coalesced and the fields of **FAKE_CHUNK_1** will be modified. Therefore, in order for the fields of **FAKE_CHUNK_1** to be preserved, it can not be *directly* before or after the <u>to be freed chunk</u>.
...
2. **FAKE_CHUNK_3**
    ...
    Speciallly crafted the **FAKE_CHUNK_3** at `0x2424` which is the start of the `data` field for `chunk2`, a.k.a. the input for `password`. Therefore, the input for `password` should be `0x2` bytes of padding, `0x4564` in little endian.
